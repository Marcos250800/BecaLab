          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
               {filtered.slice(0, 4).map(s => (
                 <div key={s.id} className="bg-slate-900 border border-slate-800 p-6 rounded-2xl hover:border-slate-700 transition-all group">
                    ...
                    <div className="flex gap-3 pt-4 border-t border-slate-800">
                       <button onClick={() => handleStartProcess(s)} className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2.5 rounded-xl font-bold flex justify-center gap-2 text-sm items-center"><PlusCircle size={16} /> Iniciar</button>
                       <a href={s.link} target="_blank" rel="noreferrer" className="px-4 py-2.5 rounded-xl text-slate-400 hover:text-white bg-slate-950 border border-slate-800 flex items-center gap-2 text-sm font-bold">Oficial <ExternalLink size={14} /></a>
                    </div>
                 </div>
               ))}
            </div>
